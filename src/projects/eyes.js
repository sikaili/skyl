...
import Eb3 from "@/projects/eyes/sound/light.mp3";
...
sampler2.add("G3", "/sound/chasing.mp3");
let delegate;
let callback;
...
export function main(_p5) {
  ...
  const playSound = (x, y) => {
    if (loading == false) {
      ...
      if (reverse1 == 0) {
        // sound[num].pan(pan);
        sampler2.triggerAttack(rate * 440);
      } else if (reverse1 == 1) {
        sampler1.triggerAttack(rate * 440);
